<template>
  ...
  >
    ...
    <CancelPopup
      ...
      @cancelAppointment="
        selectedAppointmentIndex !== null && cancel(selectedAppointmentIndex)
      "
    />
  </div>
</template>
...
import { cancelAppointment, fetchAppointments } from '@/api/bookingservice';
...
const cancel = async (index: number) => {
  ...
  if (index !== null) {
    ...
    try {
      ...
      console.error(
        'An error occured while trying to delete appointment: ',
        error
      );
    }
  }
