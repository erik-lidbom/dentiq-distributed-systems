        ...
        <div v-for="(appointment, index) in appointments" :key="index" class="bg-white border border-gray-300 rounded-lg flex justify-between items-stretch">
            ...
            <div class="flex items-center space-x-2 mt-2">
                ...
                <div class="flex items-center">
                    <button class="bg-red-500 px-5 py-2 rounded-lg text-white mr-3" @click="showCancelPopup(index)">Cancel</button>
                </div>   
            </div>
        </div>
        <CancelPopup :visible="cancelPopupVisible" @close="closeCancelPopup" @cancelAppointment="selectedAppointmentIndex !== null && cancelAppointment(selectedAppointmentIndex)" />
    </div>
</template>
