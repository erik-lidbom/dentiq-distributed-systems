<template>
  <div
    class="flex flex-col justify-between h-full p-6 sm:max-w-[250px] space-y-8 bg-dentiq-muted-lightest absolute right-0 left-0 top-0 bottom-0 z-50 w-full"
  >
    <div class="space-y-6">
      <!-- Header -->
      <div>
        <h2 class="text-dentiq-h4 font-semibold text-dentiq-muted-darkest">
          Filter by
        </h2>
      </div>
      ...
      <div class="flex flex-col space-y-4">
        <h3 class="text-dentiq-body font-semibold text-dentiq-muted-darkest">
          Services
        </h3>
        <hr />
        <div
          class="text-dentiq-body-small text-dentiq-muted-darker flex flex-wrap gap-1"
        >
          <div
...
const services = ref([
  ...
  { name: 'Oral Surgeon', selected: savedServices.includes('Oral Surgeon') },
  {
    ...
    selected: savedServices.includes('Pediatric Dentist'),
  },
  { name: 'Periodontist', selected: savedServices.includes('Periodontist') },
  {
    name: 'Prosthodontist',
    selected: savedServices.includes('Prosthodontist'),
  },
]);
